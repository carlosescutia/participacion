...
class Actores_model extends CI_Model {
    ...
    public function get_actores_dependencia($dependencia, $activo, $cve_tipo, $cve_sector) {
        $sql = 'select a.*, ta.nom_tipo, s.nom_sector from actores a left join tipo_actores ta on a.cve_tipo = ta.cve_tipo left join sectores s on a.cve_sector = s.cve_sector where a.dependencia=? and a.activo=? and a.cve_tipo=?';
        if ($cve_sector > 0) {
